...
import * as wiDataByCode from '../shared/weather-icons/weather-icons-codes.data.json';
...
export class WeatherService {
  ...
  getWeatherByСurrentLocation(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      window.navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          if (error.code === 1) {
            // if user didn't approve geolocation
            this.subscribers.city = this.getWeatherByLocation(
  ...
  private handleResponseWeatherData(responseData: any): Weather {
    ...
    const windBeaufortScale =
      this.helperService.getWindBeaufortScaleByMeterInSecond(wind.speed);
